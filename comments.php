...
<div id="comments">
	...
	<?php if (comments_open() && $recaptchaConfigured) {
		...
		$args = array(
			...
			'logged_in_as' => '<p class="logged-in-as">' . sprintf(__('You are logged in as <a href="%1$s">%2$s</a>. <a href="%3$s">Log out?</a>', 'webbooks'), esc_url(admin_url('profile.php')), esc_html($user_identity), esc_url(wp_logout_url(apply_filters('the_permalink', get_permalink())))) . '</p>',
